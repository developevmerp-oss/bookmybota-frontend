...
export default function CustomerSettingsPage() {
  ...
  return (
    <CustomerAccountLayout>
      <div className="mb-6">
        <h1 className="text-[32px] leading-tight font-extrabold text-[#111111]">My Account</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-0 md:divide-x md:divide-slate-200">
          <div className="flex items-center gap-4 md:pr-6">
            <div className="w-16 h-16 rounded-full bg-[#F7E9FF] text-[#6900AA] flex items-center justify-center text-2xl font-bold shrink-0">
              {initial}
            </div>
            <div className="min-w-0">
              <p className="font-bold text-[#111111] text-lg truncate">{displayName}</p>
          ...
          <div className="flex items-center gap-3 md:px-6">
            <div className="w-11 h-11 rounded-full bg-[#F7E9FF] text-[#6900AA] flex items-center justify-center shrink-0">
              <Calendar size={18} />
            </div>
            <div>
              <p className="text-sm text-slate-500">Member Since</p>
              <p className="font-bold text-[#6900AA]">{formatMemberSince(profile?.created_at)}</p>
            </div>
          </div>
      ...
      <h2 className="text-xl font-extrabold text-[#111111] mb-4">Account Settings</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {SETTINGS_LINKS.map((item) => (
          <Link
            ...
            className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex items-center gap-3 hover:border-[#6900AA]/30 transition-colors"
          >
            <div className="w-12 h-12 rounded-full bg-[#F7E9FF] text-[#6900AA] flex items-center justify-center shrink-0">
              <item.icon size={20} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-[#111111] flex items-center gap-2">
                {item.title}
                {item.title === "Offers" && (
      ...
      <div className="rounded-2xl bg-[#F7E9FF] px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="w-12 h-12 rounded-full bg-white text-[#6900AA] flex items-center justify-center shrink-0">
          <Headphones size={22} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-bold text-[#111111]">Need help?</p>
        ...
        <Link
          href="/customer/help"
          className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl border border-[#6900AA] bg-white text-sm font-semibold text-[#6900AA] hover:bg-white/80 shrink-0"
        >
          <Headphones size={16} />
          Contact Support
        </Link>
