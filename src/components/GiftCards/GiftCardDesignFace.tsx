...
type Props = {
  design: Pick<
    GiftCardDesign,
    "title" | "image_url" | "color_gradient" | "caption_color" | "text_color"
  >;
  className?: string;
 ...
 * - text_color → BookMyBota / gift CARD
 * - caption_color → tagline + design title
 */
export default function GiftCardDesignFace({
  ...
  const textColor = design.text_color?.trim() || "#FFFFFF";
  const captionColor = design.caption_color?.trim() || "#FFFFFF";
  ...
  return (
    ...
    >
      ...
      {/* Soft scrim so light text stays readable on bright art */}
      <div
        className="absolute inset-0 z-[1] bg-gradient-to-t from-black/60 via-black/20 to-black/35"
        aria-hidden
      />

      <div className="relative z-[2] flex h-full min-h-0 flex-col p-3 sm:p-4">
        <div>
          <p
          ...
          <p
            className={`font-bold leading-none drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] ${
          ...
          >
            gift{" "}
            <span
              className="font-bold tracking-[0.08em] text-[0.72em]"
              style={{ color: textColor }}
            >
              CARD
            </span>
        ...
        <div className="mt-auto pt-6 sm:pt-8">
          <p
            className={`font-bold leading-tight drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] ${
